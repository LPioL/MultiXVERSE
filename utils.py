...
def preprocess1(G, outpath,  directed, relabel, del_self_loops):
    """
    Graph preprocessing routine.
    """
    print('Preprocessing graph...')
...
def netpreprocess1(r_DistancematrixPPI, CLOSEST_NODES):
    ...
    neighborhood=[]
    for i in range(node_size):
        neighborhood.append(np.shape(np.extract(rawdata_DistancematrixPPI[i,:] > 0, rawdata_DistancematrixPPI[i,:]))[0])
    ...
    component=[]
    for i in range( 0,np.shape(rawdata_DistancematrixPPI)[0]):
        mini.append(np.min(rawdata_DistancematrixPPI[i,:]))
        rawdata_DistancematrixPPI[i,i]= 1/25000
        rawdata_DistancematrixPPI[i,component]=1/25000
    rawdata_DistancematrixPPI= np.transpose(rawdata_DistancematrixPPI)
    ...
    # Names of the nodes in the PPI network (a vocab in the sense of skipgram)
    nodes = list(r_DistancematrixPPI.index)
    #nodes = [int(i) for i in nodes]
    if nodes[0] == 1:
        nodes = nodes - np.ones(len(nodes), dtype=int)
    nodesstr = [str(i) for i in nodes]
